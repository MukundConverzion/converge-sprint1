...
class Linkedin:
        ...
        try:
            ...
            if len(driver.find_elements_by_class_name("no-content")) > 0:
                ...
                for article in articles:
                    ...
                    comments_divs = wait.until(lambda driver: artpage.find_elements_by_xpath(".//article"))
                    print("{} comments".format(len(comments_divs)))
    ...
    def getExperience(self):
        driver = self.driver 
        # Scroll down until experience section found
        wait = WebDriverWait(driver, 5)
        while True:
            try:
                # First look for additional positions
                if len(wait.until(lambda driver:driver.find_elements_by_xpath("//button[@class='pv-profile-section__see-more-inline link']"))) > 0:    
                    print("More positions found ...")
                    more_positions = wait.until(lambda driver:driver.find_element_by_xpath("//button[@class='pv-profile-section__see-more-inline link']"))
                    # print(more_positions)
                    action_positions = ActionChains(driver)
                    action_positions.move_to_element(more_positions).click().perform()
                    time.sleep(3)

                positions = wait.until(lambda driver:driver.find_element_by_class_name("experience-section"))
                total_positions_list = positions.find_elements_by_xpath(".//ul/li")
                print(total_positions_list)
                total_positions = [i.text for i in total_positions_list]
                total_positions = list(set(total_positions))
...
def main():
    ...
    profile_urls = {
            'Dipesh Pandey': 'https://www.linkedin.com/in/dipespandey/',
